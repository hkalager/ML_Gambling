% Code for modelling the football bets - Minimum Likelihood approach
% Codes developed by Arman Hassanniakalager
% Last modified 10 Sep. 2018 14:11 BST.
clear;
clc;
...
%   X is an N x K1 matrix of individual-specific covariates.
    ...
    [~,Predict_OOS1X2]=min(pihat_OOS1X2,[],2);
...
%     yhat = glmval(B_OvUn,X_ISOvUn,'logit');   
    ...
    [~,Predict_OOSOvUn]=min(pihat_OOSOvUn,[],2);
...
%      pihatCS = mnrval(B_CS,X_ISCS); 
    ...
    [~,Predict_OOSCS]=min(Probs_OOSCS,[],2);
    ...
    writetable(tblnew,['Predicted_Season_',num2str(ISrng(i,1)),'_',num2str(ISrng(i,2)),'_MinL.xlsx'])
end
close;
writetable(rectbl,'Logit_MinL_Results.xlsx');