...
% Last modified 06 Feb. 2018 17:49 GMT 
function tblnew=payoffestimator3(output1,oddstable,typ)
if isempty(typ)
    typ=1;
end    
tblnew=oddstable;
...
if typ==1
    ...
    PpG1x2=HCIres.*tblnew.AvgH+ACIres.*tblnew.AvgA+DCIres.*tblnew.AvgD-entrycostres;
    tblnew{:,end+1}=PpG1x2;
...
elseif typ==2
    ...
    PpGOvUn=OvCI.*tblnew.AvgOver... BbAv_2_5 means BbAv>2.5
        +UnCI.*tblnew.AvgUnder... BbAv_2_5_1 means BbAv<2.5
        -entrycostOvUn;
...
elseif typ==4
    ...
    AHdiff=(tblnew.FTHG-tblnew.FTAG)+tblnew.AHh;
    ...
    AFW=and(tblnew.pred_AH==0,AHdiff<=-.5); % Away team full win
    AHW=and(tblnew.pred_AH==0,AHdiff==-.25); % Away team half win
    ASR=and(tblnew.pred_AH==0,AHdiff==0); % Away team stake refund
    AHL=and(tblnew.pred_AH==0,AHdiff==.25); % Away team half lose
    AFL=and(tblnew.pred_AH==0,AHdiff>=.5); % Away team full lose
    
    PpGAHH=HFW.*tblnew.AvgAHH+HHW.*(1+tblnew.AvgAHH)/2+HSR.*entrycostAH+...
        HHL.*entrycostAH*.5+HFL.*0;
    PpGAHA=AFW.*tblnew.AvgAHA+AHW.*(1+tblnew.AvgAHA)/2+ASR.*entrycostAH+...
